...
public class SqlParser {
	...
	public Update parseUpdate(SQLUpdateStatement updateStatement) throws SqlParseException {
		...
		updateStatement.getTableSource().output(sb);
		update.setFromString(sb.toString().replaceAll(" ", ""));
		
		sb = new StringBuffer();
		if(updateStatement.getWhere() != null){
			update.setWhereString(updateStatement.getWhere().toString());
		}
		update.getFrom().addAll(findFrom(updateStatement.getTableSource()));
		update.setWhere(findWhere(updateStatement.getWhere()));
		update.setItems(findItems(updateStatement.getItems()));
		return update;
	}
    ...
    private Map<String, Object> findItems(List<SQLUpdateSetItem> items) throws SqlParseException {
		Map<String,Object> map = new HashMap<String,Object>();
		for(SQLUpdateSetItem item : items){
			StringBuffer sb = new StringBuffer();
			item.getColumn().output(sb);
			String key = new String(sb);
			map.put(key, parseValue(item.getValue()));
		}
		return map;
	}
