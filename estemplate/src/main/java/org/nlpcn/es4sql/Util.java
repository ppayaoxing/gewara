...
public class Util {
	...
	public static Map<String,Object> explainObj(Object obj){
		Map<String,Object> result = new HashMap<String,Object>();
		Field[] fields = obj.getClass().getDeclaredFields();
		String[] types1 = { "int", "String", "boolean", "char","float", "double", "long", "short", "byte" };
		String[] types2 = { "Integer", "String", "Boolean","Character", "Float", "Double","Long", "Short", "Byte" };
		for (int j = 0; j < fields.length; j++) {
			fields[j].setAccessible(true);
			for (int i = 0; i < types1.length; i++) {
				if (fields[j].getType().getSimpleName().equals(types1[i]) || fields[j].getType().getSimpleName().equals(types2[i])) {
					try {
						result.put(fields[j].getName(), fields[j].get(obj));
						continue;
					} catch (Exception e) {
						e.printStackTrace();
					}
