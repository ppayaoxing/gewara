...
public final class DaoServiceImpl implements DaoService {
	...
	@Override
	public <T extends BaseObject> List<T> getObjectIDList(Class<T> clazz) {
		List<T> objects = baseDao.getAllObjects(clazz);
	...
	public <T extends BaseObject> T getObjectByUkey(Class<T> clazz, String ukeyName, Serializable ukeyValue,
			boolean b) {
		return baseDao.getObjectByUkey(clazz, ukeyName, ukeyValue, b);
	}
		...
		if (entity != null) {
			try {
				PropertyUtils.setProperty(entity, property,
						(Integer) PropertyUtils.getProperty(entity, property) + num);
				baseDao.saveObject(entity);
			} catch (Exception e) {
			}
		}
	...
	private <T extends BaseObject> void addPropertiesNum(T entity, String[] propertyList, Integer[] values) {
		...
		for (String property : propertyList) {
			try {
				PropertyUtils.setProperty(entity, property,
						(Integer) PropertyUtils.getProperty(entity, property) + values[i]);
			} catch (Exception e) {
			}
			i++;
		}
	...
	public <T extends BaseObject> int updateProperties2(T entity, String properties, Serializable... values) {
		if (entity != null) {
			hibernateTemplate.refresh(entity);
			String[] propertyList = properties.split(",");
			return updateProperties2(entity, propertyList, values);
		}
		return 0;
	}
	...
	private <T extends BaseObject> int updateProperties2(T entity, String[] propertyList, Serializable... values) {
		...
		baseDao.saveObject(entity);
		Map<String, String> changeMap = changeEntry.getChangeMap(entity);
