...
public class MemcachedCacheServiceImpl extends AbstractCacheService {
	...
	public int incrementAndGet(String regionName, String key, int by, int def) {
		...
		/*
		 * if(StringUtils.isNotBlank(key)){ try{ key = getRealKey(regionName, key);
		 * return (int) this.memcachedClient.incr(key, by, def); }catch(Exception e){
		 * dbLogger.warn("", e); } } return -1;
		 */
	}
