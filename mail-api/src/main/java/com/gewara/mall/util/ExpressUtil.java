...
public class ExpressUtil {
	...
	public static ResultCode<ExpressRes> getQryRes(String nu, String com) {
		if (!StringUtils.isBlank(nu) && !StringUtils.isBlank(com)) {
			if (!StringUtils.equalsIgnoreCase(com, "ygzt") && StringUtils.length(nu) > 6
					&& !StringUtils.startsWith(nu, "0000")) {
				...
				HttpResult code = HttpUtils.postUrlAsString("https://route.showapi.com/64-19", map);
				if (!code.isSuccess()) {
					return ResultCode.getFailure(code.getStatus() + "", code.getMsg());
				} else {
					String res = code.getResponse();
					ExpressRes expRes = (ExpressRes) JsonUtils.readJsonToObject(ExpressRes.class, res);
					return !expRes.getShowapi_res_code().equals(Integer.valueOf(0))
							? ResultCode.getFailure(expRes.getShowapi_res_error())
							: ResultCode.getSuccessReturn(expRes);
				}
			} else {
	...
	public static ResultCode<ExpressRes> getCompanyQryRes() {
		...
		map.put("showapi_timestamp", curtime + "");
		map.put("showapi_sign", "simple_2703ea38b023468099941a827cb4818a");
		HttpResult code = HttpUtils.postUrlAsString("http://route.showapi.com/64-20", map);
		if (!code.isSuccess()) {
			return ResultCode.getFailure(code.getStatus() + "", code.getMsg());
		} else {
			String res = code.getResponse();
			ExpressRes expRes = (ExpressRes) JsonUtils.readJsonToObject(ExpressRes.class, res);
			return !expRes.getShowapi_res_code().equals(Integer.valueOf(0))
					? ResultCode.getFailure(expRes.getShowapi_res_error()) : ResultCode.getSuccessReturn(expRes);
		}
	}

	public static void main(String[] args) {
		ResultCode code = getQryRes("968393787473", "shentong");
		ExpressRes exp = (ExpressRes) code.getRetval();
