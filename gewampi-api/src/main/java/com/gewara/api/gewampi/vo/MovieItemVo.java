...
public class MovieItemVo extends BaseVo implements Comparable<MovieItemVo> {
	...
	public static List<MovieItemVo> getCurrent(Date date, List<MovieItemVo> playItemList) {
		...
		if (date.after(today)) {
			...
			Iterator arg4 = playItemList.iterator();

			while (arg4.hasNext()) {
				MovieItemVo mpi = (MovieItemVo) arg4.next();
	...
	public int compareTo(MovieItemVo another) {
		return this == another ? 0
				: (another == null ? 1
						: (this.playdate.after(another.playdate) ? 1
								: (this.playdate.before(another.playdate) ? -1
										: (!this.showtime.equals(another.showtime)
												? this.showtime.compareTo(another.showtime)
												: (StringUtils.isBlank(this.roomname)
														? (StringUtils.isBlank(another.roomname) ? 0 : -1)
														: (StringUtils.isBlank(another.roomname) ? 1
																: this.roomname.compareTo(another.roomname)))))));
	}
	...
	public int getMinpoint() {
		return GewampiConstant.MINPOINT.intValue();
	}

	public int getMaxpoint() {
		return GewampiConstant.MAXPOINT.intValue();
	}
