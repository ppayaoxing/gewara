...
//import com.gewara.api.service.movie.MovieGeneralmarkDetailVo;
...
public interface MovieVoService {
	...
	/**
	 * 根据影片ID集合获得影片集合 注意：该方法用于jms同步movie数据到avatar,直接查mysql数据库，不走缓存，慎用。
	 * 
	 * @param movieIds
	 * @return List<MovieVo>
	 */
	 ...
	 * @return
	 */
	ResultCode<List<MovieVo>> getMovieListBetweenReleasedate(Date startReleasedate, Date endReleasedate,
			String offlineDate);
	...
	<T extends BaseVo> ResultCode<T> getMovieVoById(Long movieid);

	<T extends BaseVo> ResultCode<T> getCinemaVoById(Long cinemaid);

	<T extends BaseVo> ResultCode<List<T>> getMovieGeneralmarkDetail(long movieId);
}
