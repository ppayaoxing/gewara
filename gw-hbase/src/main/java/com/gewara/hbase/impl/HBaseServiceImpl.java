...
public class HBaseServiceImpl implements HBaseService, InitializingBean {
	...
	/**
	 * AggregationClient用于hbase中实现简单聚合计算,基于{@link Configuration}建立
	 */
	...
	public void setupHbase() {
		...
		// HConnectionManager.HConnectionImplementation.class.getName());//默认获取的实现类,无需配置
		...
		new Thread(new Runnable(){
			...
			public void run() {
				...
				/*
				 * getHConnection是个阻塞方法,只要返回值返回,就代表环境已经建立完毕,无需使用客户端等待一定的时间
				 */
	...
	/**
	 * 自#Result解析数据到一行(一个 map 扁平化各行的数据字段)
	 * 
	 * @param row
	 * @return
	 */
	protected Row getRowData(Result row) {
		...
		// XXX 仅解析关注指定列族的数据, gewara 使用的列族为 DF
	...
	public List<byte[]> getRowidListByRange(String tableName, FilterBuilder fb, Long starttime, Long endtime, int maxnum, String column) {
		...
		if (StringUtils.isNotBlank(column) && scanBuilder.isEmptyCondition() && HbaseData.supportTimeRange(tableName)) {
			scan.addColumn(columnFamily, column.getBytes());// not exists不需要其他数据
		}
	...
	/**
	 * 这是一个功能比较弱且容易出问题的 API, 故而使用时,请关注注释.
	 * 
	 * for query: 默认是做相等比较,如果operator传入值,且值为'<>',则做不相等比较 for
	 * likeQuery:只做相等比较,与operator传入值无关
	 * 
	 * 关于Range,处理区间的策略是'[...)',前闭后开
	 * 
	 * @param tablename
	 * @param query
	 * @param likeQuery
