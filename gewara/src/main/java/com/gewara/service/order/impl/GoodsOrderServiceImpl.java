...
public class GoodsOrderServiceImpl extends GewaOrderServiceImpl implements GoodsOrderService {
    ...
    public ErrorCode<GoodsOrder> addSportGoodsOrder(SportGoods goods, Member member, String mobile, int quantity, String address) {
        if (goods.getMaxbuy() < quantity) {
            return ErrorCode.getFailure(ApiConstant.CODE_SIGN_ERROR, "每单最多购买:" + goods.getMaxbuy());
        }
    ...
    public ErrorCode<GoodsOrder> addTicketGoodsOrder(TicketGoods goods, Member member, String mobile, Integer quantity, Long disid, Long priceid, ApiUser partner, String ukey) {
        if (priceid == null) {
            return ErrorCode.getFailure(ApiConstant.CODE_SIGN_ERROR, "价格错误！");
        }
        if (quantity == null || quantity < 1) {
            return ErrorCode.getFailure(ApiConstant.CODE_SIGN_ERROR, "数量有错误！");
        }
        if (!goods.hasBooking()) {
            return ErrorCode.getFailure(ApiConstant.CODE_SIGN_ERROR, "场次已关闭购票！");
        }
    ...
    private ErrorCode<List<BuyItem>> getBuyItem(List<GoodsCommand> commandList) {
        ...
        for (GoodsCommand command : commandList) {
            ...
            if (goods == null || !goods.hasBooking()) {
                return ErrorCode.getFailure(ApiConstant.CODE_SIGN_ERROR, goods.getGoodsname() + "场次已关闭购票！");
            }
            if (categoryid == null) {
            ...
            if (StringUtils.equals(command.getTag(), GoodsConstant.CHECK_GOODS_PRICE)) {
                ...
                if (discount == null) {
                    return ErrorCode.getFailure(ApiConstant.CODE_SIGN_ERROR, "购买套票错误！");
                }
                goodsPrice = baseDao.getObject(GoodsPrice.class, discount.getGspid());
                quantity = command.getQuantity() * discount.getQuantity();
            }
            if (goodsPrice == null) {
                return ErrorCode.getFailure(ApiConstant.CODE_SIGN_ERROR, "购票价格错误！");
            }
            if (!goodsPrice.hasBooking()) {
                return ErrorCode.getFailure(ApiConstant.CODE_SIGN_ERROR, "购卖失败，价格库存不足！");
            }
            ...
            if (goods.getMaxbuy() < goodsQuantity) {
                return ErrorCode.getFailure(ApiConstant.CODE_SIGN_ERROR, goods.getGoodsname() + "购票数不能大于" + goods.getMaxbuy());
            }
    ...
    public ErrorCode<GoodsOrder> addTrainingGoodsOrder(Long goodsId, Long gspId, Integer quantity, String mobile, String infoList, Member member, ApiUser partner, String ukey) {
        ...
        if (goods == null) {
            return ErrorCode.getFailure(ApiConstant.CODE_SIGN_ERROR, "不存在此商品！");
        }
    ...
    private ErrorCode<GoodsOrder> addBaseGoodsOrder(BaseGoods goods, Member member, String mobile, int quantity, ApiUser partner, String ukey) {
        if (!goods.hasBooking()) {
            return ErrorCode.getFailure(ApiConstant.CODE_SIGN_ERROR, "场次或物品已关闭，不能购票！");
        }
        if (!ValidateUtil.isMobile(mobile)) {
            return ErrorCode.getFailure(ApiConstant.CODE_SIGN_ERROR, "手机号格式错误！");
        }
        if (quantity <= 0) {
            return ErrorCode.getFailure(ApiConstant.CODE_SIGN_ERROR, "购卖数量错误！");
        }
    ...
    public ErrorCode<GoodsOrder> addActivityGoodsOrder(ActivityGoods goods, Member member, ApiUser partner, String mobile, int quantity, String realname, String address, Timestamp jointime) {
        if (StringUtils.isBlank(realname)) {
            return ErrorCode.getFailure(ApiConstant.CODE_SIGN_ERROR, "联系人不能为空！");
        }
        if (goods.isNeedDeliver() && StringUtils.isBlank(address)) {
            return ErrorCode.getFailure(ApiConstant.CODE_SIGN_ERROR, "地址不能为空！");
        }
        if (isOverQuantity(goods)) {
            return ErrorCode.getFailure(ApiConstant.CODE_SIGN_ERROR, "商品库存数量不足，请联系管理员！");
        }
        if (jointime == null) {
            return ErrorCode.getFailure(ApiConstant.CODE_SIGN_ERROR, "参加日期不能为空！");
        }
        if (goods.getMaxbuy() < quantity) {
            return ErrorCode.getFailure(ApiConstant.CODE_SIGN_ERROR, "每单最多购买:" + goods.getMaxbuy());
        }
        ...
        if (allowaddnum != null) {
            if (allowaddnum < quantity) {
                return ErrorCode.getFailure(ApiConstant.CODE_SIGN_ERROR, "下单人数过多，您可等15分钟内未支付的订单释放名额！");
            }
            goods.setAllowaddnum(allowaddnum - quantity);
        }
        int limitnum = goods.getLimitnum() + quantity;
        if (limitnum > goods.getQuantity()) {
            return ErrorCode.getFailure(ApiConstant.CODE_SIGN_ERROR, "支付名额已满，活动已结束！");
        }
        Long memberid = null;
        memberid = member.getId();
    ...
    /**
     * 查询商品订单列表,根据商品id和memberid
     *
     * @param goodsId  商品id
     * @param memberId 用户id
     * @param status   状态
     * @param like     是否模糊查询
     * @param order    排序字段 true:"addtime"  false:"paidtime"
     * @param maxnum   查询列表数量
     * @return
     */
    @Override
            ...
            if (goods instanceof Goods) {
                if (GoodsConstant.GOODS_TAG_BMH.equals(goods.getTag())) {
                    ...
                    if (sport != null) {
                        ...
                        return ErrorCode.getFailure("物品关联运动场馆有误");
                    }
                }
    ...
    public ErrorCode usePoint(Long orderId, Long memberId, int usePoint) {
        ...
        if (info.getPointvalue() < usePoint) {
            return ErrorCode.getFailure("您的积分不够！");
        }
        List<BuyItem> itemList = baseDao.getObjectListByField(BuyItem.class, "orderid", order.getId());
        List<Long> idList = BeanUtil.getBeanPropertyList(itemList, "relatedid", true);
        List<BaseGoods> goodsList = baseDao.getObjectList(BaseGoods.class, idList);
        ...
        if (!openPointPay) {
            return ErrorCode.getFailure("你选择的商品不支持积分优惠！");
        }
        if (maxpoint < usePoint) {
            return ErrorCode.getFailure("您使用的积分超出上限" + maxpoint);
        }
    ...
    public ErrorCode<GoodsOrder> addGoodsOrderByBuyItem(TicketOrder torder) {
        ...
        if (order != null) {
            return ErrorCode.getFailure("已经添加过！");
        }
    ...
    private ErrorCode<GoodsOrderContainer> useElecCard(GoodsOrder order, ElecCard card, Long memberid) {
        if (!order.isNew()) {
            return ErrorCode.getFailure("订单状态错误（" + order.getStatusText() + "）！");
        }
        BaseGoods goods = baseDao.getObject(BaseGoods.class, order.getGoodsid());
        if (goods instanceof TicketGoods) {
            TicketGoods ticketGoods = (TicketGoods) goods;
            if (!card.validTag(ticketGoods.getCategory())) {
                return ErrorCode.getFailure("本券不能在该版块使用！");
            }
        } else if (goods instanceof TrainingGoods) {
            //运动劵票
        ...
        if (!isSupportCard) {
            return ErrorCode.getFailure("该场次不支持该券的使用！");
        }
    ...
    private ErrorCode getDiscount(GoodsOrder order, List<BuyItem> itemList, List<BaseGoods> goodsList, ElecCard card, Long memberid) {
        //1、判断卡是否有效
        if (!card.available()) {
            return ErrorCode.getFailure("此兑换券已经用完或失效！");
        }
        if (order.sureOutPartner()) {//非Gewa商家
            ...
            if (card.getPossessor() != null) {
                return ErrorCode.getFailure("此卡必须登录后使用");
            }
        ...
        if (batch.getCardtype().equals(PayConstant.CARDTYPE_C) ||
            ...
            if (goodsPrice == null) {
                return ErrorCode.getFailure("兑换出错！");
            }
            if (item.getDisid() != null) {
        ...
        if (amount <= 0) {
            return ErrorCode.getFailure("使用此兑换券得不到任何优惠，请看使用说明！");
        }
    ...
    public ErrorCode<BaseGoods> checkOrder(GoodsOrder order) {
        ...
        for (BaseGoods baseGoods : goodsList) {
            if (StringUtils.isNotBlank(baseGoods.getExpressid())) {
                ...
                if (orderAddress == null) {
                    return ErrorCode.getFailure("请返回上一步，填写收件人及收件地址！");
                } else {
                    break;
                }
            }
        ...
        if (goods instanceof TicketGoods) {
            ...
            if (goods.getQuantity() != null && goods.getQuantity() != 0 && limitnum > goods.getQuantity()) {
                return ErrorCode.getFailure(ApiConstant.CODE_SIGN_ERROR, "支付名额已满！");
            }
        }
