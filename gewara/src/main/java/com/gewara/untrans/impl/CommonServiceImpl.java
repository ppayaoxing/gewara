...
import com.gewara.model.common.*;
import com.gewara.model.content.*;
...
public class CommonServiceImpl implements CommonService {
    ...
    private List<GewaCommend> getGewaCommendList(String citycode, String countycode, String signname, Long parentid, String tag, boolean isGtZero,
        ...
        if (asc) {
            query.addOrder(
                    Order.asc(StringUtils.isNotBlank(order)
                            && ClassUtils.hasMethod(GewaCommend.class, "get" + StringUtils.capitalize(order)) ? order : "ordernum"));
        } else {
            query.addOrder(
                    Order.desc(StringUtils.isNotBlank(order)
                            && ClassUtils.hasMethod(GewaCommend.class, "get" + StringUtils.capitalize(order)) ? order : "ordernum"));
        }
    ...
    public void initGewaCommendList(String group, RelatedHelper rh, List<GewaCommend> gcList) {
        ...
        Object relate2 = null;
        String tag = "";
        List<Long> activityIdList = Lists.newLinkedList();
        Map<Long, GewaCommend> map = Maps.newHashMap();
