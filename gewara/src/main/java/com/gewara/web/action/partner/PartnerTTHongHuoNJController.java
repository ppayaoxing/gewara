 ...
 */
@Controller
public class PartnerTTHongHuoNJController extends BasePartnerController {
	...
	private ApiUser getApiUser(){
		return daoService.getObject(ApiUser.class, PartnerConstant.PARTNER_TTHONGHUO_NJ);
	}
	private static final int pageSize = 10;
	@RequestMapping("/partner/tthonghuonj/index.xhtml")
	public String index(String moviename, ModelMap model,String encQryStr, 
		...
		if(StringUtils.isBlank(ukey)) PartnerUtil.setUkCookie(response, config.getBasePath() + "partner/tthonghuonj/");
		...
		PageUtil pageUtil = new PageUtil(rowsCount , pageSize, pageNo,"partner/tthonghuonj/index.xhtml", true, true);
		...
		return "partner/tthonghuonj/index.vm";
	}
	
	@RequestMapping("/partner/tthonghuonj/movieDetail.xhtml")
	public String movieDetail(Long movieid, String encQryStr,@CookieValue(required=false,value="ukey") String ukey,Date fyrq, 
		...
		opiList(partner, movieid, fyrq, "partner/tthonghuonj/step1.vm", model);
		...
		model.put("paramsStr", "");
		model.put("encQryStr", encQryStr);
		return "partner/tthonghuonj/movieDetail.vm";
	}
	
	//第二步：选择座位
	@RequestMapping("/partner/tthonghuonj/chooseSeat.xhtml")
	public String chooseSeat(Long mpid,String encQryStr, ModelMap model){
		...
		return chooseSeat(apiUser, mpid, reqParamMap.get("userId"), "partner/tthonghuonj/chooseSeat.vm", model);
	}
	
	@RequestMapping("/partner/tthonghuonj/seatPage.xhtml")
	...
	//第三步:锁座位，加订单
	@RequestMapping("/partner/tthonghuonj/addOrder.xhtml")
	...
	//第四步：确认订单去支付（重新查看）
	@RequestMapping("/partner/tthonghuonj/showOrder.xhtml")
	public String showOrder(Long orderId, String encQryStr,  ModelMap model, 
		...
		return showOrder(ukey, orderId, partner, "partner/tthonghuonj/confirmOrder.vm", model);
	}
	//第四步：确认订单去支付
	@RequestMapping(method=RequestMethod.POST,value="/partner/tthonghuonj/saveOrder.xhtml")
