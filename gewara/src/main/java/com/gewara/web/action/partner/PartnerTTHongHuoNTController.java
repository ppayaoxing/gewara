 ...
 */
@Controller
public class PartnerTTHongHuoNTController extends BasePartnerController {
	...
	private ApiUser getApiUser(){
		return daoService.getObject(ApiUser.class, PartnerConstant.PARTNER_TTHONGHUO_NT);
	}
	private static final int pageSize = 10;
	@RequestMapping("/partner/tthonghuont/index.xhtml")
	public String index(String moviename, ModelMap model,String encQryStr, 
		...
		if(StringUtils.isBlank(ukey)) PartnerUtil.setUkCookie(response, config.getBasePath() + "partner/tthonghuont/");
		...
		PageUtil pageUtil = new PageUtil(rowsCount , pageSize, pageNo,"partner/tthonghuont/index.xhtml", true, true);
		...
		return "partner/tthonghuont/index.vm";
	}
	
	@RequestMapping("/partner/tthonghuont/movieDetail.xhtml")
	public String movieDetail(Long movieid, String encQryStr,@CookieValue(required=false,value="ukey") String ukey,Date fyrq, 
		...
		opiList(partner, movieid, fyrq, "partner/tthonghuont/step1.vm", model);
		...
		model.put("paramsStr", "");
		model.put("encQryStr", encQryStr);
		return "partner/tthonghuont/movieDetail.vm";
	}
	
	//第二步：选择座位
	@RequestMapping("/partner/tthonghuont/chooseSeat.xhtml")
	public String chooseSeat(Long mpid,String encQryStr, ModelMap model){
		...
		return chooseSeat(apiUser, mpid, reqParamMap.get("userId"), "partner/tthonghuont/chooseSeat.vm", model);
	}
	
	@RequestMapping("/partner/tthonghuont/seatPage.xhtml")
	...
	//第三步:锁座位，加订单
	@RequestMapping("/partner/tthonghuont/addOrder.xhtml")
	...
	//第四步：确认订单去支付（重新查看）
	@RequestMapping("/partner/tthonghuont/showOrder.xhtml")
	public String showOrder(Long orderId, String encQryStr,  ModelMap model, 
		...
		return showOrder(ukey, orderId, partner, "partner/tthonghuont/confirmOrder.vm", model);
	}
	//第四步：确认订单去支付
	@RequestMapping(method=RequestMethod.POST,value="/partner/tthonghuont/saveOrder.xhtml")
