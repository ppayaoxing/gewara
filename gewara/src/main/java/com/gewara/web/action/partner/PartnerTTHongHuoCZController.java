 ...
 */
@Controller
public class PartnerTTHongHuoCZController extends BasePartnerController {
	...
	private ApiUser getApiUser(){
		return daoService.getObject(ApiUser.class, PartnerConstant.PARTNER_TTHONGHUO_CZ);
	}
	private static final int pageSize = 10;
	@RequestMapping("/partner/tthonghuocz/index.xhtml")
	public String index(String moviename, ModelMap model,String encQryStr, 
		...
		if(StringUtils.isBlank(ukey)) PartnerUtil.setUkCookie(response, config.getBasePath() + "partner/tthonghuocz/");
		...
		PageUtil pageUtil = new PageUtil(rowsCount , pageSize, pageNo,"partner/tthonghuocz/index.xhtml", true, true);
		...
		return "partner/tthonghuocz/index.vm";
	}
	
	@RequestMapping("/partner/tthonghuocz/movieDetail.xhtml")
	public String movieDetail(Long movieid, String encQryStr,@CookieValue(required=false,value="ukey") String ukey,Date fyrq, 
		...
		opiList(partner, movieid, fyrq, "partner/tthonghuocz/step1.vm", model);
		...
		model.put("paramsStr", "");
		model.put("encQryStr", encQryStr);
		return "partner/tthonghuocz/movieDetail.vm";
	}
	
	//第二步：选择座位
	@RequestMapping("/partner/tthonghuocz/chooseSeat.xhtml")
	public String chooseSeat(Long mpid,String encQryStr, ModelMap model){
		...
		return chooseSeat(apiUser, mpid, reqParamMap.get("userId"), "partner/tthonghuocz/chooseSeat.vm", model);
	}
	
	@RequestMapping("/partner/tthonghuocz/seatPage.xhtml")
	...
	//第三步:锁座位，加订单
	@RequestMapping("/partner/tthonghuocz/addOrder.xhtml")
	...
	//第四步：确认订单去支付（重新查看）
	@RequestMapping("/partner/tthonghuocz/showOrder.xhtml")
	public String showOrder(Long orderId, String encQryStr,  ModelMap model, 
		...
		return showOrder(ukey, orderId, partner, "partner/tthonghuocz/confirmOrder.vm", model);
	}
	//第四步：确认订单去支付
	@RequestMapping(method=RequestMethod.POST,value="/partner/tthonghuocz/saveOrder.xhtml")
