...
@Controller
public class PartnerTaiZhouController extends BasePartnerController {
	@Autowired@Qualifier("commonService")
	private CommonService commonService;
	public void setCommonService(CommonService commonService) {
		this.commonService = commonService;
	}
	private ApiUser getTaiZhou(){
		return daoService.getObject(ApiUser.class, PartnerConstant.PARTNER_TAIZHOU);
	}
	private static final int pageSize = 10;
	@RequestMapping("/partner/taizhou/index.xhtml")
	public String index(String moviename, ModelMap model,String encQryStr, 
		...
		ApiUser partner = getTaiZhou();
		if(StringUtils.isBlank(ukey)) PartnerUtil.setUkCookie(response, config.getBasePath() + "partner/taizhou/");
		...
		PageUtil pageUtil = new PageUtil(rowsCount , pageSize, pageNo,"partner/taizhou/index.xhtml", true, true);
		...
		return "partner/taizhou/index.vm";
	}
	
	@RequestMapping("/partner/taizhou/movieDetail.xhtml")
	public String movieDetail(Long movieid, String encQryStr,@CookieValue(required=false,value="ukey") String ukey,Date fyrq, 
		...
		opiList(partner, movieid, fyrq, "partner/taizhou/step1.vm", model);
		...
		model.put("paramsStr", "");
		model.put("encQryStr", encQryStr);
		return "partner/taizhou/movieDetail.vm";
	}
	
	//第二步：选择座位
	@RequestMapping("/partner/taizhou/chooseSeat.xhtml")
	public String chooseSeat(Long mpid,String encQryStr, ModelMap model){
		...
		ApiUser apiUser = getTaiZhou();
		return chooseSeat(apiUser, mpid, reqParamMap.get("userId"), "partner/taizhou/chooseSeat.vm", model);
	}
	
	@RequestMapping("/partner/taizhou/seatPage.xhtml")
	...
	//第三步:锁座位，加订单
	@RequestMapping("/partner/taizhou/addOrder.xhtml")
	...
	//第四步：确认订单去支付（重新查看）
	@RequestMapping("/partner/taizhou/showOrder.xhtml")
	public String showOrder(Long orderId, String encQryStr,  ModelMap model, 
		...
		return showOrder(ukey, orderId, partner, "partner/taizhou/confirmOrder.vm", model);
	}
	//第四步：确认订单去支付
	@RequestMapping(method=RequestMethod.POST,value="/partner/taizhou/saveOrder.xhtml")
